        ...
        <div className="flex-1 min-w-0">
          ...
          {activeSlot === 'skin' && (
            <div className="grid grid-cols-3 gap-3">
              {SKIN_TONES.map(s => (
                <button key={s.id} onClick={() => save({ skin_tone: s.id })} disabled={saving}
          ...
          {activeSlot === 'eyes' && (
            <div className="fn-card p-5" style={{ borderRadius: 4 }}>
              ...
              <button onClick={() => save({ eye_color: eyeColorInput })} disabled={saving}
          ...
          {activeSlot === 'mouth' && (
            <div className="grid grid-cols-3 gap-3">
              {MOUTH_STYLES.map(m => (
                <button key={m.id} onClick={() => save({ mouth_style: m.id })} disabled={saving}
          ...
          {activeSlot === 'hair' && (
            <div className="grid grid-cols-4 gap-3">
              {HAIR_STYLES.map(h => (
                <button key={h.id} onClick={() => save({ hair_style: h.id })} disabled={saving}
                  className="fn-card flex flex-col items-center p-3 transition-all"
          ...
          {activeSlot === 'hair_color' && (
            <div className="grid grid-cols-2 gap-3">
              {HAIR_COLORS.map(h => (
                <button key={h.id} onClick={() => save({ hair_color: h.id })} disabled={saving}
          ...
          {activeSlot === 'shirt' && (
            <div className="grid grid-cols-3 gap-3">
              {SHIRT_COLORS.map(c => (
                <button key={c.id} onClick={() => save({ shirt_color: c.id })} disabled={saving}
                  className="fn-card flex flex-col items-center p-3 transition-all"
          ...
          {activeSlot === 'pants' && (
            <div className="grid grid-cols-3 gap-3">
              {PANTS_COLORS.map(c => (
                <button key={c.id} onClick={() => save({ pants_color: c.id })} disabled={saving}
                  className="fn-card flex flex-col items-center p-3 transition-all"
              ...
              <div>
                {items.length === 0 ? (
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                    <button onClick={() => unequip(activeSlot)} disabled={saving}
                      className="fn-card flex flex-col items-center p-3 transition-all"
                    ...
                    {items.map(item => {
                      ...
                      return (
                        <button key={item.id} onClick={() => equip(activeSlot, item.id)} disabled={saving}
                          className="fn-card flex flex-col items-center p-3 transition-all"
